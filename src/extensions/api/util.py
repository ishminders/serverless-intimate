...
def build_chat_parameters(body):
    ...
    generate_params = {
        ...
        'encoder_repetition_penalty': float(body.get('encoder_repetition_penalty', 1.0)),
        'top_k': int(body.get('top_k', 0)),
        'chat_prompt_size': int(body.get('top_k', 50)),
        'chat_generation_attempts': int(body.get('top_k', 1)),
        'stop_at_newline': bool(body.get('stop_at_newline', True)),
        'min_length': int(body.get('min_length', 0)),
        'no_repeat_ngram_size': int(body.get('no_repeat_ngram_size', 0)),
        'num_beams': int(body.get('num_beams', 1)),
        'penalty_alpha': float(body.get('penalty_alpha', 0)),
